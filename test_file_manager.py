...
def test_file_manager():
    ...
    res = session.post(
        f"{BASE_URL}/process",
        json={"dataset_name": "D1", "original_filename": "dummy1.csv"},
    )
    ...
    res = session.post(
        f"{BASE_URL}/process",
        json={"dataset_name": "D2", "original_filename": "dummy2.csv"},
    )
    ...
    if len(files_after) == len(files) - 2:
        ...
        print(
            f"FAILURE: File count mismatch (Expected {len(files) - 2}, got {len(files_after)})"
        )
