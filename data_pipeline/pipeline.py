...
class DataPipeline:
    ...
    def load(self, source, **kwargs) -> "DataPipeline":
        """
        ...
        **kwargs : dict
        ...
        print(
            f"✅ Loaded dataset: {self.raw_df.shape[0]:,} rows × {self.raw_df.shape[1]} columns"
        )
    ...
    def clean(
        ...
        if handle_outliers:
            self.cleaner.handle_outliers(method=outlier_method, action=outlier_action)
    ...
    def engineer_features(
        ...
        self.engineer = FeatureEngineer(
            df, target_col=self.target_col, problem_type=self.problem_type
        )
    ...
    def _print_pipeline_summary(self) -> None:
        ...
        if self.raw_df is not None:
            print("\n📊 Data Transformation:")
            print(
                f"   Raw:     {self.raw_df.shape[0]:,} rows × {self.raw_df.shape[1]} columns"
            )

        if self.cleaned_df is not None:
            print(
                f"   Cleaned: {self.cleaned_df.shape[0]:,} rows × {self.cleaned_df.shape[1]} columns"
            )

        if self.final_df is not None:
            print(
                f"   Final:   {self.final_df.shape[0]:,} rows × {self.final_df.shape[1]} columns"
            )
        ...
        if self.final_df is not None:
            ...
            print(
                f"   • Ready for modeling: {'Yes' if missing == 0 else 'No (handle remaining missing)'}"
            )
    ...
    def save_data(
        ...
        if save_cleaned and self.cleaned_df is not None:
            if format == "csv":
                ...
                self.cleaned_df.to_parquet(
                    output_path / "cleaned_data.parquet", index=False
                )
        ...
        if save_final and self.final_df is not None:
            if format == "csv":
                ...
                self.final_df.to_parquet(
                    output_path / "final_data.parquet", index=False
                )
    ...
    def train_model(
        ...
        self.trainer = ModelTrainer(
            df, target_col=t_col, problem_type=p_type, raw_df=self.raw_df
        )
    ...
    def generate_markdown_report(self, output_path: str) -> str:
        ...
        """
        report = []
        report.append("# Mini Data Clean Tool - Model Report")
        report.append(
        ...
        if self.raw_df is not None:
            report.append(
                f"- **Raw Dataset:** {self.raw_df.shape[0]:,} rows × {self.raw_df.shape[1]} columns"
            )
        if self.cleaned_df is not None:
            report.append(
                f"- **Cleaned Dataset:** {self.cleaned_df.shape[0]:,} rows × {self.cleaned_df.shape[1]} columns"
            )
        ...
        if self.model_results and "comparison" in self.model_results:
            ...
            report.append(
                f"| **Raw Baseline** | {raw_score}% ({metric_name}) | **{raw_grade}** ({raw_pts}/100) |"
            )
            ...
            report.append(
                f"| **Cleaned Model** | **{clean_score}%** ({metric_name}) | *(See Dashboard)* |"
            )
        ...
        report.append("\n## 3. Key Observations")
        report.append(
            "> This model report was generated automatically by the Mini Data Clean Tool."
        )
