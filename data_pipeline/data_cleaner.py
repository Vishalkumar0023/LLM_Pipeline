...
class DataCleaner:
    ...
    def __init__(self, df: pd.DataFrame):
        ...
        for col in self.df.columns:
            ...
            elif pd.api.types.is_integer_dtype(self.df[col]) and hasattr(
                self.df[col].dtype, "numpy_dtype"
            ):
                self.df[col] = self.df[col].astype(self.df[col].dtype.numpy_dtype)
            elif pd.api.types.is_float_dtype(self.df[col]) and hasattr(
                self.df[col].dtype, "numpy_dtype"
            ):
                self.df[col] = self.df[col].astype(self.df[col].dtype.numpy_dtype)
    ...
    def remove_duplicates(
        ...
        if not dupes.empty:
            for idx in dupes.index[:2]:
                ...
                row_str = (
                    ", ".join([str(x) for x in self.df.loc[idx].values[:3]]) + "..."
                )
        ...
        if removed > 0:
            ...
            self.transformations.append(
                {"operation": "remove_duplicates", "rows_removed": removed}
            )
                    ...
                    self.row_changes.append(
                        {
                            ...
                            "column": col,
                            "old_value": "NaN",
                            "new_value": round(float(val_used), 4)
                            if isinstance(val_used, (float, int))
                            else str(val_used),
    ...
    def fix_data_types(
        self, type_mapping: Optional[Dict[str, str]] = None, infer_types: bool = True
    ) -> "DataCleaner":
        """
                ...
                if col in self.df.columns:
                    try:
                        ...
                        self.cleaning_log.append(
                            f"Could not convert {col} to {dtype}: {e}"
                        )
        ...
        if changes:
            ...
            self.cleaning_log.append(msg)
            self.transformations.append(
                {"operation": "fix_data_types", "changes": changes}
            )
    ...
    def clean_categorical_values(
        ...
        if changes:
            ...
            self.cleaning_log.append(msg)
            self.transformations.append(
                {"operation": "clean_categorical", "changes": changes}
            )
        ...
        if method == "label":
            ...
            for col in columns:
                if self.df[col].nunique() > max_categories:
                    self.cleaning_log.append(
                        f"Skipping OneHot for {col}: >{max_categories} categories"
                    )
                    continue
        ...
        if columns is None:
            ...
            for col in self.df.select_dtypes(include=["object", "string"]).columns:
                ...
                sample = (
                    ...
                    .dropna()
                    .astype(str)
                    .sample(min(20, len(self.df)), random_state=42)
                )
                if (
                    sample.str.contains(r"[\$\€\£\,kKmMbB]").any()
                    and sample.str.contains(r"\d").all()
                ):
                    columns.append(col)
            ...
            if valid_converted > 0:
                ...
                if valid_mask.any():
                    ...
                    for idx in sample_indices:
                        val_old = self.df.loc[
                            idx, col
                        ]  # This is already the NEW value in self.df
    ...
    def rename_columns(self, mapping: Dict[str, str]) -> "DataCleaner":
        ...
        if valid_mapping:
            ...
            self.cleaning_log.append(msg)
            self.transformations.append(
                {"operation": "rename_columns", "mapping": valid_mapping}
            )

        return self

    def extract_regex_feature(
        self, source_col: str, pattern: str, new_col_name: str
    ) -> "DataCleaner":
        r"""
        ...
        try:
            ...
            extracted = (
                self.df[source_col].astype(str).str.extract(pattern, expand=False)
            )
    ...
    def drop_columns(
        ...
        if cols_to_drop:
            ...
            self.cleaning_log.append(msg)
            self.transformations.append(
                {"operation": "drop_columns", "columns": cols_to_drop}
            )
    ...
    def validate_quality(self) -> Dict[str, Any]:
        ...
        completeness_score = max(
            0, 40 * (1 - missing_pct * 2)
        )  # Penalize missing heavily
        ...
        for col in self.df.columns:
            if self.df[col].nunique() <= 1:
                ...
                report["warnings"].append(
                    f"Column '{col}' is constant (1 unique value)"
                )
    ...
    def generate_suggestions(self) -> List[Dict[str, str]]:
        ...
        for col in numeric_cols:
            if self.df[col].nunique() < 10:
                continue  # Skip categorical-like
        ...
        for col in self.df.columns:
            if col.lower() in ["id", "uuid", "guid", "index"] or (
                ...
                and pd.api.types.is_string_dtype(self.df[col])
            ):
                suggestions.append(
                    {
                        "column": col,
