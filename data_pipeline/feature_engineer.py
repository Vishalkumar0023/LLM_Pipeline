...
class FeatureEngineer:
        ...
        for col in columns:
            ...
            if use_method == "onehot":
                ...
                self.df.loc[mask, col] = le.fit_transform(
                    self.df.loc[mask, col].astype(str)
                )
    ...
    def scale_features(
        self, method: str = "standard", columns: Optional[List[str]] = None
    ) -> "FeatureEngineer":
        """
        ...
        if cols_to_scale:
            ...
            self.transformations.append(
                f"{method.capitalize()} scaled {len(cols_to_scale)} numeric features"
            )
    ...
    def create_datetime_features(
        ...
        for col in columns:
            ...
            self.transformations.append(
                f"Extracted {len(features)} features from '{col}'"
            )
    ...
    def create_polynomial_features(
        ...
        self.transformations.append(
            f"Created {len(new_features)} polynomial features (degree={degree})"
        )
    ...
    def create_binned_features(
        ...
        self.transformations.append(
            f"Created binned features for {len(columns)} columns"
        )

        return self

    def compute_feature_importance(self, n_features: int = 20) -> Dict[str, float]:
        """
    ...
    def drop_low_importance_features(
        self, threshold: float = 0.01, keep_n: Optional[int] = None
    ) -> "FeatureEngineer":
        """
        ...
        if keep_n:
            ...
            features_to_keep = [
                f for f, imp in self.feature_importance.items() if imp >= threshold
            ]
        ...
        if cols_to_drop:
            ...
            self.transformations.append(
                f"Dropped {len(cols_to_drop)} low-importance features"
            )
    ...
    def drop_low_variance_features(self, threshold: float = 0.01) -> "FeatureEngineer":
        ...
        try:
            ...
            if cols_to_drop:
                self.df = self.df.drop(columns=cols_to_drop)
                self.transformations.append(
                    f"Dropped {len(cols_to_drop)} low-variance features"
                )
        except:
            pass
    ...
    def drop_highly_correlated(self, threshold: float = 0.95) -> "FeatureEngineer":
        """
        ...
        if cols_to_drop:
            self.df = self.df.drop(columns=cols_to_drop)
            self.transformations.append(
                f"Dropped {len(cols_to_drop)} highly correlated features (r > {threshold})"
            )

        return self

    def handle_class_imbalance(
        self, method: str = "smote", sampling_strategy: Union[str, float] = "auto"
    ) -> "FeatureEngineer":
        """
        ...
        try:
            if method == "smote":
                ...
                self.transformations.append(
                    f"Applied SMOTE: {len(y)} → {len(y_resampled)} samples"
                )
                ...
                self.df = pd.concat(dfs).reset_index(drop=True)
                self.transformations.append("Undersampled majority classes")
    ...
    def auto_evolve(self, max_new_features: int = 10) -> "FeatureEngineer":
        ...
        top_features = [f for f in self.feature_importance.keys() if f in numeric_cols][
            :5
        ]
        ...
        try:
            ...
            new_feat_importance = {
                f: self.feature_importance.get(f, 0) for f in created_features
            }

            # Sort by importance
            sorted_new = sorted(
                new_feat_importance.items(), key=lambda x: x[1], reverse=True
            )

            # Keep top max_new_features
            keep_features = [
                f for f, imp in sorted_new[:max_new_features] if imp > 0.001
            ]
            ...
            if keep_features:
                self.transformations.append(
                    f"Auto-evolved {len(keep_features)} new interaction features: {', '.join(keep_features[:3])}..."
                )
