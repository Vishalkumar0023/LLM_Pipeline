...
class ModelTrainer:
        ...
        for hint in TARGET_HINTS:
            for col in cols:
                if hint.lower() == col.lower() or col.lower().startswith(hint.lower()):
                    self.target_col = col
                    self.log.append(
                        f"Auto-detected target column: '{col}' (matched hint '{hint}')"
                    )
                    return col
            ...
            if self.problem_type == "regression":
                ...
                if y.dtype == "object" or y.dtype.name == "category":
                    try:
                        ...
                        self.log.append(
                            "Auto-corrected problem type to 'classification' (target is string)"
                        )
    ...
    def _drop_junk_columns(self):
        ...
        if cols_to_drop:
            ...
            self.log.append(
                f"Dropped {len(cols_to_drop)} junk columns: {cols_to_drop[:5]}{'...' if len(cols_to_drop) > 5 else ''}"
            )
    ...
    def _drop_high_correlation(self, threshold: float = 0.9):
        ...
        if to_drop:
            ...
            self.log.append(
                f"Dropped {len(to_drop)} highly correlated features (r > {threshold})"
            )
    ...
    def _extract_datetime_parts(self):
        ...
        if dt_cols:
            self.log.append(
                f"Extracted date parts from {len(dt_cols)} datetime columns"
            )
    ...
    def prepare_features(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if non_numeric:
            ...
            self.log.append(
                f"Dropped {len(non_numeric)} non-numeric columns before training"
            )
        ...
        y = y.fillna(
            y.mode()[0] if self.problem_type == "classification" else y.median()
        )
        ...
        self.log.append(
            f"Scaled {len(self.feature_names)} features with StandardScaler"
        )
    ...
    def detect_imbalance(self) -> Dict[str, Any]:
        ...
        info = {
            ...
            "class_distribution": {
                str(k): round(v * 100, 1) for k, v in counts.items()
            },
        }
    ...
    def _get_models_and_params(self) -> Dict[str, Dict]:
        ...
        n_classes = (
            len(np.unique(self.y)) if self.problem_type == "classification" else 0
        )
    ...
    def train_all_models(self) -> Dict[str, Dict]:
        ...
        if (
            ...
            try:
                ...
                self.log.append(
                    f"Applied SMOTE: {len(self.y)} → {len(y_train)} samples"
                )
        ...
        for name, config in model_defs.items():
            ...
            try:
                ...
                cv_scores = cross_validate(
                    best_estimator, X_train, y_train, cv=cv, scoring=scoring, n_jobs=1
                )
    ...
    def get_best_model(self) -> Dict[str, Any]:
        ...
        primary_metric = (
            "f1_weighted" if self.problem_type == "classification" else "r2"
        )
        ...
        if self.best_model_name:
            ...
            self.log.append(
                f"Best model: {self.best_model_name} ({primary_metric}={best_score:.4f})"
            )

        return {
            "name": self.best_model_name,
            "metrics": self.best_metrics,
            "params": self.cv_results.get(self.best_model_name, {}).get(
                "best_params", {}
            ),
        }
    ...
    def _compute_reliability(
        self, n_rows: int, cv_std: float, metric_val: float
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if self.target_col not in df.columns:
                ...
                if self.target_col.lower() in col_map:
                    ...
                    self.log.append(
                        f"Mapped raw target '{found_col}' to '{self.target_col}'"
                    )
                else:
                    # Fail gracefully
            ...
            memory_mb = round(
                self.original_df.memory_usage(deep=True).sum() / (1024 * 1024), 2
            )
    ...
    def run_full_comparison(self) -> Dict[str, Any]:
        ...
        if (
            "best_model" in advanced_dashboard
            and "metrics" in advanced_dashboard["best_model"]
        ):
        ...
        advanced_dashboard["comparison"] = {
            ...
            "optimization": {
                ...
                "cleaned_time": advanced_dashboard.get(
                    "time_taken", 0
