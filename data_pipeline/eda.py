...
class EDAAnalyzer:
        ...
        for col in categorical_cols:
            ...
            if len(value_counts) > 50:
                self.insights.append(
                    f"High cardinality in '{col}': {len(value_counts)} unique values"
                )
    ...
    def correlation_analysis(
        self, method: str = "pearson", threshold: float = 0.7
    ) -> pd.DataFrame:
    ...
    def target_analysis(self) -> Dict[str, Any]:
        ...
        if target.dtype in [np.number, "int64", "float64"]:
            ...
            analysis["class_balance"] = round(
                value_counts.min() / value_counts.max(), 3
            )

            if analysis["class_balance"] < 0.5:
                self.insights.append(
                    f"Class imbalance detected in target: ratio = {analysis['class_balance']}"
                )
    ...
    def plot_boxplots(self, columns=None, figsize=(15, 10), save_path=None):
        ...
        plt.suptitle(
            "Feature Boxplots (Outlier Detection)", fontsize=14, fontweight="bold"
        )
        plt.tight_layout()
    ...
    def plot_categorical(self, columns=None, figsize=(15, 10), save_path=None):
        ...
        plt.suptitle(
            "Categorical Feature Distributions", fontsize=14, fontweight="bold"
        )
        plt.tight_layout()
    ...
    def plot_target_relationships(self, figsize=(15, 10), save_path=None):
        ...
        for idx, col in enumerate(numeric_cols):
            ax = axes[idx]
            ax.scatter(
                self.df[col], self.df[self.target_col], alpha=0.5, s=10, color="#4C72B0"
            )
            ax.set_xlabel(col)
    ...
    def run_full_analysis(self, show_plots=True, save_plots=False, output_dir=None):
        ...
        if show_plots:
            save_path = lambda name: (
                f"{output_dir}/{name}.png" if save_plots and output_dir else None
            )
            ...
            if self.target_col:
                self.plot_target_relationships(
                    save_path=save_path("target_relationships")
                )
        ...
        return {"stats": self.stats, "insights": self.insights}
