            ...
            [
                {
                    ...
                    "content": "Gradient descent is an optimization algorithm used to minimize the loss function. It iteratively adjusts parameters in the direction of steepest descent. The learning rate controls the step size.",
                },
                {
                    "topic": "Backpropagation",
                    "content": "Backpropagation is the algorithm used to compute gradients in neural networks. It applies the chain rule to calculate how each weight contributes to the overall error.",
                },
            ],
            f,
            indent=2,
...
def test_layer2_chunking(docs):
    ...
    report(
        "Chunks have 'estimated_tokens'", all("estimated_tokens" in c for c in chunks)
    )
...
def test_layer5_versioning(data):
    ...
    meta1 = registry.register(
        data, version="v1.0.0", description="Initial test dataset", tags=["test"]
    )
...
def test_layer6_finetune_config():
    ...
    config = FineTuneConfig(
        model_name="meta-llama/Meta-Llama-3-8B", method="lora", backend="trl"
    )
...
def test_layer6_monitoring():
    ...
    monitor.log_run(
        "v1.0",
        {"loss": 0.8, "perplexity": 15.0, "bleu": 0.25, "rouge_1": 0.40},
        model_name="llama-3-8b",
    )

    monitor.log_run(
        "v1.1",
        {"loss": 0.5, "perplexity": 10.0, "bleu": 0.35, "rouge_1": 0.52},
        model_name="llama-3-8b",
    )
    ...
    # Report
    report_md = monitor.generate_report(os.path.join(TEST_DIR, "eval_report.md"))
    report("Report generated", len(report_md) > 100)
    report(
        "Report file saved", os.path.exists(os.path.join(TEST_DIR, "eval_report.md"))
    )
